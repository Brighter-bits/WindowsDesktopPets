...
void Turn(SDL_Renderer* renderer, Textures &Textures, SDL_Event &event, bool &running); // Turn the sprite around and has them go in a different direction
void Render(SDL_Renderer* renderer, SDL_Event &event, bool &running, Textures &Textures, WindowManager* windowow = nullptr); // The render function which slaps the sprite onto the window and renders everything
void Vertical(SDL_Renderer* renderer, Textures &Textures, SDL_Event &event, bool &running, WindowManager &window); // Tells the sprite to go up or down
...
int WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    SDL_SetWindowOpacity(window.window, 1);
    ...
    while (running) {
        ...
        if (Timer % 20 == 0) {
            ...
            if ((rng.out() == 5) && (Timer % 200 == 0)) { // Random change for the Sprite to go up or down
                Vertical(renderer, Textures, event, running, window);
            }
...
void Turn(SDL_Renderer* renderer, Textures &Textures, SDL_Event &event, bool &running) {
    bool Complete = false;
    unsigned int Timer = 0;
    ...
    while (!Complete) {
        ...
        Render(renderer, event, running, Textures);
...
void Render(SDL_Renderer* renderer, SDL_Event &event, bool &running, Textures &Textures, WindowManager* windowow) {
...
void Vertical(SDL_Renderer* renderer, Textures &Textures, SDL_Event &event, bool &running, WindowManager &window) {
    int Complete = 0;
    unsigned int Timer = 0;
    RandomInt rng(0, 1);
    bool Up = NULL;
    ...
    while (Complete <= 2) {
        ...
        Render(renderer, event, running, Textures, &window); // Render the changes
        Timer += 1;
    }
